...
from vocabulary import (
    ...
    WORDS,
    WORDS_BY_ID,
    example_has_persian_translation,
    has_persian_meaning,
    make_example,
)
from ui import (
...
class ProgressStore:
    ...
    def add_words_to_learning(self, user_id: int, word_ids: list[int]) -> None:
        if not word_ids:
            return
        with self.connect() as connection:
            connection.executemany(
                ...
                ON CONFLICT(user_id, word_id)
                DO UPDATE SET
                    status = CASE WHEN status = 'learned' THEN status ELSE 'learning' END,
                    updated_at = CURRENT_TIMESTAMP
                """,
                [(user_id, word_id) for word_id in word_ids],
            )
...
async def send_study_card(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    entry = pick_study_word(STORE, user_id)
    if not entry:
...
async def placement(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await send_placement_intro(update.message)
...
async def begin_placement(user_id: int, chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    await start_placement(user_id, context)
    await context.bot.send_message(
        chat_id=chat_id,
        text="آزمون شروع شد.",
        reply_markup=placement_cancel_keyboard(),
    )
...
async def send_placement_question_to_chat(
    ...
    message = await context.bot.send_message(
        ...
        text=(
            ...
            f"زمان: {PLACEMENT_TIMEOUT_SECONDS} ثانیه\n\n"
...
async def cancel_placement_for_chat(
    user_id: int,
    chat_id: int,
    context: ContextTypes.DEFAULT_TYPE,
) -> None:
    cancel_placement_timeout(context, user_id)
    context.application.user_data.get(user_id, {}).pop("placement", None)
    await context.bot.send_message(
        chat_id=chat_id,
        text="آزمون لغو شد.",
        reply_markup=main_keyboard(),
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "placement_start":
        await send_placement_intro(query.message)
        return

    if action == "placement_begin":
        await query.edit_message_text("آزمون شروع شد.")
        await begin_placement(user_id, query.message.chat_id, context)
        return
